...
def getLoginInfo(): # function getting ID, PASSWORD for sending request
    print("앱 등록을 위해서는 ID, PASSWORD가 필요합니다.")
    id=input("ID: ")
    passwd=input("PASSWORD: ")
    return id, passwd

def AppRegisterRequest(Serv_url, Name,RequireGPU, DockerImage, Description, Arguments, OpenPorts, id, passwd): # function requesting registration
    data = {
        'Name': Name,
    ...
    response = requests.post(Serv_url, data=data, auth=(id,passwd), verify=False) # post request, added auth
    return response
...
RequireGPU=input("GPU 요청여부를 입력하세요(사용:True, 사용하지 않음:False).\n입력하지 않으면 False으로 처리됩니다:  ")
...
OpenPorts=input("포트를 입력하세요.\n입력하지 않으면 기본포트(12300)으로 처리됩니다: ")
response = AppRegisterRequest(Serverurl, Name,RequireGPU, DockerImage, Description, Arguments, OpenPorts, ID, PASSWORD)
